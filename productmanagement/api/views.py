...
from productmanagement.utils import Util, FieldMaps
...
def blog_in_industy(request):
    try:
        ...
        productmanagements = User.objects.filter(
                            industry__title=payload['industry_name'].replace('-', ' ')).all()

        all_posts = []

        for productmanagement in productmanagements:
            posts = list(Post.objects.filter(author=productmanagement).all().values())
